  ...
  return (
    <SafeAreaView className="flex-1 bg-[#FAFAFA]" edges={['top', 'left', 'right']}>
      ...
      >
          ...
          <View className="flex-row flex-wrap justify-between gap-y-3">
            ...
            <View style={{ width: statCardWidth, height: 152 }} className="bg-white border border-[#E2E8F0] rounded-[28px] p-5 justify-between">
              <View className="w-10 h-10 rounded-2xl bg-[#FFF3E8] items-center justify-center">
                <TrendUp size={20} color="#F67300" variant="Linear" />
              </View>
              <View>
            ...
            <View style={{ width: statCardWidth, height: 152 }} className="bg-white border border-[#E2E8F0] rounded-[28px] p-5 justify-between">
              <View className="w-10 h-10 rounded-2xl bg-[#E8F8F0] items-center justify-center">
                <TickCircle size={20} color="#1DD75B" variant="Linear" />
              </View>
              <View>
            ...
            <View style={{ width: statCardWidth, height: 152 }} className="bg-white border border-[#E2E8F0] rounded-[28px] p-5 justify-between">
              <View className="w-10 h-10 rounded-2xl bg-[#FDE8E8] items-center justify-center">
                <CloseCircle size={20} color="#E61026" variant="Linear" />
              </View>
              <View>
            ...
            <View style={{ width: statCardWidth, height: 152 }} className="bg-white border border-[#E2E8F0] rounded-[28px] p-5 justify-between">
              <View className="w-10 h-10 rounded-2xl bg-[#F5F5F5] items-center justify-center">
                <MinusCirlce size={20} color="#888" variant="Linear" />
              </View>
              <View>
        ...
        <Text className="font-black text-[#333333] mb-1" style={{ fontSize: 20, lineHeight: 36 }}>Test Result</Text>
        ...
        {MOCK_RESULTS.filter(item => {
          ...
          if (activeFilter.startsWith('Correct')) return item.status === 'Correct';
          if (activeFilter.startsWith('Wrong')) return item.status === 'Wrong';
          if (activeFilter.startsWith('Skipped')) return item.status === 'Skipped';
          return true;
        }).map((item, index) => {
          ...
          return (
            <View key={item.id} className="bg-white rounded-[20px] p-5 mb-4 border border-[#F2EEF4]">
              <TouchableOpacity onPress={() => toggleExpand(item.id)} className="mb-3" activeOpacity={0.7}>
                    ...
                    <View className="flex-1 flex-row items-center justify-between">
                      <View className="flex-row items-center gap-2">
                        ...
                        <View className={`flex-row items-center px-2 py-0.5 rounded-full ${
                          ...
                          {isWrong && <CloseCircle size={12} color="#FB2C36" variant="Linear" style={{ marginRight: 4 }} />}
                          {isCorrect && <TickCircle size={12} color="#2A9A46" variant="Linear" style={{ marginRight: 4 }} />}
                ...
                <Text className="font-semibold text-[#333]" style={{ fontSize: 15, lineHeight: 20.63 }}>
                  {item.question}
                </Text>
              </TouchableOpacity>

              {isExpanded && (
                <View className="mt-4 pt-5 border-t border-[#F2EEF4]">
                  {/* Your Answer */}
                  <View className="rounded-[16px] py-4 px-5 mb-3 border flex-row items-start" style={{
                    ...
                    <View className={`w-8 h-8 rounded-full items-center justify-center mr-3 mt-1 ${
                      isWrong ? 'bg-[#FB2C36]' : isCorrect ? 'bg-[#2A9A46]' : 'bg-[#888]'
                    }`}>
                      {isWrong && <CloseCircle size={18} color="#FFF" variant="Linear" />}
                      {isCorrect && <TickCircle size={18} color="#FFF" variant="Linear" />}
                      {isSkipped && <MinusCirlce size={18} color="#FFF" variant="Linear" />}
                    </View>
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="font-semibold pr-2" style={{
                        fontSize: 14,
                        lineHeight: 21,
                  ...
                  {(isWrong || isSkipped) && item.correctAnswer && (
                    <View className="rounded-[16px] py-4 px-5 border flex-row items-start" style={{
                      backgroundColor: 'rgba(42,154,70,0.05)',
                      borderColor: 'rgba(42,154,70,0.4)',
                    }}>
                      <View className="w-8 h-8 rounded-full bg-[#2A9A46] items-center justify-center mr-3 mt-1">
                        <TickCircle size={18} color="#FFF" variant="Linear" />
                      </View>
                      ...
                      <View className="flex-1">
                        ...
                        <Text className="font-semibold text-[#2A9A46] pr-2" style={{ fontSize: 14, lineHeight: 21 }}>{item.correctAnswer}</Text>
                      </View>
                    </View>
                  )}
                </View>
